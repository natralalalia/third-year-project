...
void neuron_do_timestep_update(timer_t time, uint timer_count) { // EXPORTED
    ...
    for (index_t neuron_index = 0; neuron_index < n_neurons; neuron_index++) {
         ...
         bool spike = neuron_impl_do_timestep_update(neuron_index, external_bias, ready_to_decode, time, official_activation_1, official_activation_2);
         ...
         if(time % 16 == 0 && number_of_spikes != 0 && neuron_index == n_neurons - 1) { // for middle and output layers
            ...
            if(number_of_spikes == 1) {
                ...
                if(neuron_indexes_spikes[0] == 1) official_activation_1 = 17 - neuron_times_spikes[0]; // Activation < 16
                else official_activation_1 = 16*neuron_indexes_spikes[0];                              // Activation % 16 == 0
                printf("Official activation is %d as only one neuron spiked!\n", official_activation_1);
            } else {
                if(number_of_spikes == 2) {
                    printf("Number of spikes is 2. Maybe each of the two activations from the previous layer was a multiple of 16, or less than 16 \n");
                    official_activation_2 = 0;
